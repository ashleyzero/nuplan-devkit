...
from nuplan.planning.training.modeling.models.diffusion_utils import TransformerConcatLinear, VarianceSchedule, SampleSelector
from nuplan.planning.training.modeling.models.diffusion_dynamics import Dynamics
...
class DiffusionModel(TorchModuleWrapper):
    ...
    def get_metrics(self, features: FeaturesType) -> Optional[Dict[str, float]]:
        ...
        context = self.encode(critical_agent_features)
        controls = self.sample(context)
        predicted_poses = self.dynamics.integrate(
            controls=controls, present_agent=critical_agent_features.get_present_critical_agent(), dt=self.dt)

        target_poses = critical_agent_features.get_future_critical_agent_poses()
        avg_displacement_error = torch.norm(predicted_poses[..., :2] - target_poses[..., :2], dim=-1).mean()
        final_displacement_error = torch.norm(predicted_poses[:, -1, :2] - target_poses[:, -1, :2], dim=-1).mean()

        return {"avg_displacement_error": avg_displacement_error , "final_displacement_error": final_displacement_error}
    ...
    def forward(self, features: FeaturesType) -> TargetsType:
        ...
        for i in range(self.sample_selector.num_samples):
            controls = self.sample(context)
            samples.append(self.dynamics.integrate_and_differentiate(
                controls=controls, present_agent=critical_agent_features.get_present_critical_agent(), dt=self.dt))
        ...
        for i in range(critical_agent_features.batch_size):
            poses, velocities, _, _ = samples[best_indices[i]]
            best_poses.append(poses[i])
            best_velocities.append(velocities[i])
        return {"trajectory": Trajectory(data=torch.stack(best_poses)) , "velocity": Trajectory(data=torch.stack(best_velocities))}